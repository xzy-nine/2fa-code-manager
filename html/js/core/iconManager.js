...
// 从全局作用域获取依赖
const IconManagerUtils = (typeof window !== 'undefined') ? window.GlobalScope?.CoreUtils : global.GlobalScope?.CoreUtils;
...
const IconManagerModule = {
     ...
     */
    async getIconUrl(url, element = null) {        if (!url) return DEFAULT_ICON;

        const domain = IconManagerUtils.getDomain(url);
    ...
    async tryFetchIcon(iconUrl, sourceUrl) {
        try {
            ...
            const base64data = await IconManagerUtils.blobToBase64(blob);
    ...
    async addPaddingToSmallIcon(iconData) {
        return new Promise((resolve) => {
            const img = new Image();            img.onload = () => {
                // 创建一个新画布，尺寸为32x32
                const canvas = IconManagerUtils.createElement('canvas', '', { width: '32', height: '32' });
     ...
     */
    async handleIconError(img, fallbackIcon = DEFAULT_ICON) {        img.src = fallbackIcon;
        
        const originalUrl = img.dataset.originalUrl;
        if (originalUrl) {
            const domain = IconManagerUtils.getDomain(originalUrl);
            if (iconCache.has(domain)) {
                iconCache.delete(domain);
                
                try {
                    await chrome.storage.local.remove(originalUrl);                } catch (error) {
    ...
    async setIconForElement(img, url) {
        ...
        try {
            ...
            img.dataset.processingUrl = url;            img.onerror = () => this.handleIconError(img);
            ...
            img.src = DEFAULT_ICON;            delete img.dataset.processingUrl;
     ...
     */
    async setCustomIcon(url, base64Image) {        try {
            await chrome.storage.local.set({ [url]: base64Image });
            await this.cacheIcon(IconManagerUtils.getDomain(url), base64Image);        } catch (error) {
            Notification.showErrorMessage('保存图标失败: ' + error);
        }
    },
    ...
    async cacheIcon(domain, iconData) {
        ...
        try {
            await chrome.storage.local.set({ [domain]: iconData });        } catch (error) {
    ...
    generateInitialBasedIcon(domain) {
        ...
        const canvas = IconManagerUtils.createElement('canvas', '', { width: '64', height: '64' });
        const ctx = canvas.getContext('2d');
     ...
     */
    stripFallbackPrefix(iconData) {        if (iconData && iconData.startsWith(FALLBACK_ICON_PREFIX)) {
...
if (typeof window !== 'undefined') {
  ...
  window.GlobalScope.IconManager = IconManagerModule;
} else if (typeof global !== 'undefined') {
  global.GlobalScope = global.GlobalScope || {};
  global.GlobalScope.IconManager = IconManagerModule;
}
